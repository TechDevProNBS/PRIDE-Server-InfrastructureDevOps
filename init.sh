...
RESOURCE_GROUP_NAME=pride
STORAGE_ACCOUNT_NAME=pride$(date +%s) CONTAINER_NAME=pride
EXISTING_RESOURCE_GROUP_NAME=$(az group list --query "[?name=='pride'] | [0].name" -o tsv)
...
EXISTING_STORAGE_ACCOUNT_NAME=$(az storage account list --resource-group pride --query "[?starts_with(name, 'pride')] | [0].name" -o tsv)
...
AZURE_STORAGE_CONTAINER=$(az storage container list --account-name $AZURE_STORAGE_ACCOUNT --account-key $AZURE_STORAGE_KEY --query "[?name=='pride'] | [0].name" -o tsv)
